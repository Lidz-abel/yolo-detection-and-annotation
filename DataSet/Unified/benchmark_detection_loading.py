...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark raw and chunked detection loading.")
    parser.add_argument(
        "--raw-manifest",
        type=Path,
    ...
    parser.add_argument("--formats", nargs="+", default=["pickle", "npz", "pt", "safetensors", "hdf5", "msgpack"])
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("/home/lidz/YOLO/DataSet/Unified/benchmark_results.csv"),
    )
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("/home/lidz/YOLO/DataSet/Unified/benchmark_results.json"),
    )
...
def main() -> None:
    ...
    results = []
    manifest_stem = args.raw_manifest.stem
    pin_memory_options = [str_to_bool(value) for value in args.pin_memory_options]
    persistent_workers_options = [str_to_bool(value) for value in args.persistent_workers_options]
