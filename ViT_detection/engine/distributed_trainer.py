...
def train_one_epoch_ddp(
    ...
    for batch_index, (images, targets) in enumerate(loader, start=1):
        ...
        if scaler is not None and scaler.is_enabled():
            ...
            scaler.update()
            did_optimizer_step = scaler.get_scale() >= scale_before
        else:
            total_loss.backward()
            if grad_clip_norm and float(grad_clip_norm) > 0.0:
                torch.nn.utils.clip_grad_norm_(model.parameters(), float(grad_clip_norm))
        ...
        if model_ema is not None and did_optimizer_step:
    ...
    skipped_tensor = torch.tensor([float(skipped_optimizer_steps)], dtype=torch.float64, device=device)
    _all_reduce_max(skipped_tensor)
    metrics["skipped_optimizer_steps"] = int(skipped_tensor.item())
    metrics["global_step"] = global_step
    if rank == 0:
