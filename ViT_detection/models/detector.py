...
from models.backbone import (
    BaselineBackbone,
    HybridViTDetectionBackbone,
    MultiScaleBaselineBackbone,
    ResNet18LikeBackbone,
    ViTDualScaleBackbone,
)
...
class YOLOv0Baseline(nn.Module):
    ...
    def _build_backbone(model_name, width_mult, depth_mult, use_residual, feature_levels=None):
        ...
        if model_name in {"vit_dual_scale", "vit_dual_scale_three_box"}:
            return ViTDualScaleBackbone(
                width_mult=width_mult,
                depth_mult=depth_mult,
                feature_levels=feature_levels,
            )
        if model_name in {"hybrid_vit_detection", "hybrid_vit_p3p4p5_three_box"}:
            return HybridViTDetectionBackbone(
                width_mult=width_mult,
                depth_mult=depth_mult,
                feature_levels=feature_levels,
            )
