...
def build_optimizer(model, train_cfg):
    ...
    if bool(train_cfg.get("param_groups_enabled", False)):
        decay_params = []
        no_decay_params = []
        backbone_lr_mult = float(train_cfg.get("backbone_lr_mult", 1.0))
        transformer_lr_mult = float(train_cfg.get("transformer_lr_mult", backbone_lr_mult))
        neck_lr_mult = float(train_cfg.get("neck_lr_mult", 1.0))
        ...
        for name, parameter in model.named_parameters():
            ...
            if clean_name.startswith("backbone."):
                lr_mult = transformer_lr_mult if ".encoder." in clean_name else backbone_lr_mult
            elif clean_name.startswith("neck."):
                lr_mult = neck_lr_mult
            elif clean_name.startswith("head."):
                lr_mult = head_lr_mult
            target = no_decay_params if parameter.ndim <= 1 or clean_name.endswith(".bias") else decay_params
            target.append({"params": [parameter], "lr": lr * lr_mult})
        param_groups = []
        if decay_params:
            param_groups.extend({**group, "weight_decay": weight_decay} for group in decay_params)
        if no_decay_params:
            param_groups.extend({**group, "weight_decay": 0.0} for group in no_decay_params)
    else:
    ...
    if scheduler_name == "cosine_warmup":
        ...
        def lr_lambda(epoch_index):
            if warmup_epochs > 0 and epoch_index < warmup_epochs:
                progress = float(epoch_index) / float(warmup_epochs)
            ...
            cosine_factor = 0.5 * (1.0 + math.cos(math.pi * float(cosine_index) / float(cosine_epochs)))
...
def build_criterion(data_cfg, model_cfg, loss_cfg, feature_levels):
