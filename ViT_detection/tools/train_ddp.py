...
def parse_prefixes(raw_value) -> tuple[str, ...]:
    """Parse a comma-separated module-prefix list from config."""
    if raw_value is None:
        return ()
    return tuple(item.strip() for item in str(raw_value).split(",") if item.strip())


def has_any_prefix(name: str, prefixes: tuple[str, ...]) -> bool:
    return any(name == prefix or name.startswith(f"{prefix}.") for prefix in prefixes)


def load_partial_init_checkpoint(model, train_cfg: dict, rank: int) -> dict:
    """Load same-shape weights from a mature detector checkpoint for first-run quality."""
    checkpoint_raw = str(train_cfg.get("init_checkpoint", "")).strip()
    ...
    include_prefixes = parse_prefixes(train_cfg.get("init_include_prefixes", ""))
    exclude_prefixes = parse_prefixes(train_cfg.get("init_exclude_prefixes", ""))
    ...
    for key, value in source_state.items():
        ...
        if include_prefixes and not has_any_prefix(clean_key, include_prefixes):
            skipped_prefix += 1
            continue
        if exclude_prefixes and has_any_prefix(clean_key, exclude_prefixes):
            skipped_prefix += 1
            continue
        if clean_key not in target_state:
            skipped_missing += 1
            continue
        if tuple(target_state[clean_key].shape) != tuple(value.shape):
            skipped_shape += 1
            continue
    ...
    summary = {
        ...
        "matched": len(matched_state),
        "skipped_shape": skipped_shape,
        "skipped_prefix": skipped_prefix,
        "skipped_missing": skipped_missing,
        "include_prefixes": ",".join(include_prefixes),
        "exclude_prefixes": ",".join(exclude_prefixes),
    }
    if rank == 0:
        print("partial init checkpoint:", summary, flush=True)
    return summary


def set_module_prefixes_trainable(model, prefixes: tuple[str, ...], trainable: bool) -> int:
    """Freeze or unfreeze parameters whose names match the configured prefixes."""
    if not prefixes:
        return 0
    changed = 0
    for name, parameter in model.named_parameters():
        clean_name = name.removeprefix("module.")
        if has_any_prefix(clean_name, prefixes):
...
def build_grad_scaler(enabled: bool, device) -> torch.amp.GradScaler:
    """Create an AMP grad scaler only when CUDA mixed precision is active."""
...
def format_history_line(item: dict) -> str:
    item = {**item, "skipped_optimizer_steps": item.get("skipped_optimizer_steps", 0)}
    return (
...
def main():
    ...
    try:
        ...
        sync_batchnorm = bool(train_cfg.get("sync_batchnorm", False)) and world_size > 1 and device.type == "cuda"
        if sync_batchnorm:
        ...
        if resume_checkpoint is not None:
            ...
            init_summary = load_partial_init_checkpoint(base_model, train_cfg, rank)

        param_stats = count_parameters(base_model)
        output_shape = describe_model_output(base_model, int(data_cfg["image_size"]), device)

        freeze_prefixes = parse_prefixes(train_cfg.get("freeze_modules", ""))
        freeze_modules_epochs = max(0, int(train_cfg.get("freeze_modules_epochs", 0)))
        find_unused_parameters = bool(train_cfg.get("ddp_find_unused_parameters", False)) or (
            bool(freeze_prefixes) and freeze_modules_epochs > 0
        )
        ...
        scaler = build_grad_scaler(use_amp, device)
        ema_enabled = bool(train_cfg.get("ema_enabled", False))
        model_ema = ModelEMA(
            base_model,
            decay=float(train_cfg.get("ema_decay", 0.9998)),
            warmup_updates=int(train_cfg.get("ema_warmup_updates", 2000)),
        ...
        if is_rank0(rank):
            ...
            print("amp enabled:", use_amp, flush=True)
            print("grad clip norm:", grad_clip_norm, flush=True)
            print("ema enabled:", ema_enabled, flush=True)
            print("sync batchnorm:", sync_batchnorm, flush=True)
            print("partial init:", init_summary, flush=True)
            print("freeze modules:", ",".join(freeze_prefixes) if freeze_prefixes else "", flush=True)
            print("freeze modules epochs:", freeze_modules_epochs, flush=True)
            print("ddp find unused parameters:", find_unused_parameters, flush=True)
            ...
            update_metadata(
                ...
                ema_decay=float(train_cfg.get("ema_decay", 0.9998)),
                sync_batchnorm=sync_batchnorm,
                partial_init=init_summary,
                freeze_modules=",".join(freeze_prefixes),
                freeze_modules_epochs=freeze_modules_epochs,
        ...
        try:
            for epoch_index in range(start_epoch, epochs + 1):
                if freeze_prefixes and freeze_modules_epochs > 0:
                    freeze_active = epoch_index <= freeze_modules_epochs
                    affected_params = set_module_prefixes_trainable(base_model, freeze_prefixes, not freeze_active)
                    if is_rank0(rank) and (epoch_index == start_epoch or epoch_index == freeze_modules_epochs + 1):
                        state_text = "frozen" if freeze_active else "unfrozen"
                        print(
                            f"[epoch {epoch_index:03d}] {state_text} modules "
                            f"{','.join(freeze_prefixes)} | affected params = {affected_params}",
                            flush=True,
                        )
                ...
                if is_rank0(rank):
                    if val_metrics is not None and val_metrics["total_loss"] < best_val_loss:
                        ...
                        best_model = model_ema.ema if model_ema is not None else model
                        save_checkpoint_ddp(best_model, run_info["output_dir"] / "best.pth")

                    save_checkpoint_ddp(model, run_info["output_dir"] / "last.pth")
                    if model_ema is not None:
                        save_checkpoint_ddp(model_ema.ema, run_info["output_dir"] / "last_ema.pth")
                    if epoch_index % save_interval_epochs == 0 or epoch_index == epochs:
                        ...
                        if model_ema is not None:
                            save_checkpoint_ddp(model_ema.ema, run_info["output_dir"] / f"epoch_{epoch_index:03d}_ema.pth")
                ...
                barrier()
                if scheduler is not None and train_metrics.get("skipped_optimizer_steps", 0) < train_metrics["optimizer_steps"]:
                    scheduler.step()
        ...
        if is_rank0(rank):
            ...
            summary_lines = [
                ...
                f"ema_enabled = {ema_enabled}",
                f"sync_batchnorm = {sync_batchnorm}",
                f"partial_init = {init_summary}",
                f"freeze_modules = {','.join(freeze_prefixes)}",
                f"freeze_modules_epochs = {freeze_modules_epochs}",
                f"ddp_find_unused_parameters = {find_unused_parameters}",
            ...
            summary_lines.extend(
                [
                    ...
                    f"last_ema_checkpoint = {run_info['output_dir'] / 'last_ema.pth' if ema_enabled else ''}",
