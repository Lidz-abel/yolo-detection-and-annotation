...
\begin{enumerate}[leftmargin=2em]
  \item 完成当前 COCO-only + \texttt{lambda\_noobj=1.0}；
  \item 若需要继续大规模训练，应优先切换到 \texttt{tools/train\_ddp.py}，避免继续使用低效的 DataParallel；
  \item 使用修复后的代码重跑必要的 smoke 和小样本诊断；
  \item 对比 \texttt{dynamic\_topk=1}；
  \item 再考虑 \texttt{dynamic\_anchor\_shape\_cost=0.25/0.5}；
